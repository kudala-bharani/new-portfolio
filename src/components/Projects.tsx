...
import { ExternalLink, Github, Globe, Users, Cloud, FolderOpen, Zap, Play, CreditCard, Chrome } from 'lucide-react';
...
const Projects: React.FC = () => {
  const mainProjects = [
    {
      title: 'Finance Flow',
      description: 'A modern, full-stack personal finance management application. Track income, expenses, budget goals, and savings with an intuitive interface featuring real-time analytics, multi-account support, and smart currency conversion.',
      technologies: ['React', 'TypeScript', 'Node.js', 'PostgreSQL', 'Prisma', 'Express', 'Tailwind CSS'],
      githubUrl: 'https://github.com/kudala-bharani/Finanace-Flow-Updated',
      demoUrl: '#',
      liveUrl: 'https://finance-flow-frontend-o34t.onrender.com/',
      icon: CreditCard,
      color: 'blue',
      highlights: ['Dashboard & Analytics', 'Multi-Account Support', 'Budget Goals', 'Currency Conversion'],
      buttons: ['live', 'demo', 'code']
    },
    {
      title: 'GPREC Quiz Application',
      description: 'A customized online quiz app with strong anti-cheating security, time-limited assessments, and automatic scoring. Features tab-switching prevention, copy/paste disable, and calculates best three scores out of five quizzes.',
      technologies: ['Spring Boot', 'Spring JPA', 'H2 Database', 'HTML', 'CSS', 'JavaScript', 'Thymeleaf'],
      githubUrl: 'https://github.com/kudala-bharani/GPREC-QUIZ',
      demoUrl: '#',
      icon: Zap,
      color: 'green',
      highlights: ['Anti-Cheating Security', 'Automatic Scoring', 'Time Limits', 'Section Notice Boards'],
      buttons: ['code', 'demo']
    }
  ];
  ...
  const smallProjects = [
    {
      ...
      description: 'A Chrome extension that generates tailored, downloadable PDF cover letters using Google Gemini API. Simply save your API key and profile, paste a job posting, and get an instant professional cover letter.',
      ...
      liveUrl: 'https://chromewebstore.google.com/detail/cover-letter-generator/biahiggabhkbbdnoabfjlbeljojgjohl',
      icon: Chrome,
      color: 'purple',
    ...
    },
    {
      ...
      description: 'A comprehensive responsive website for an orphanage supporting orphan boys. Features modern design, donation system, and volunteer management.',
        ...
        {project.buttons.map((buttonType: string, btnIndex: number) => {
          ...
          return (
            <a
              key={btnIndex}
              href={href}
              className={`flex items-center gap-2 bg-slate-700 text-white px-3 py-2 rounded-lg font-medium transition-all duration-200 ${getButtonColor(project.color)} hover:scale-105 flex-1 justify-center text-sm`}
            >
              {icon}
              {label}
            </a>
          ...
          <div className="flex flex-col gap-4">
            {smallProjects.map((project, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`bg-gradient-to-br ${getColorClasses(project.color)} p-4 rounded-xl border shadow-sm hover:shadow-lg transition-all duration-300 hover:scale-105 group flex-1`}
                >
                  <div className="flex items-center gap-2 mb-3">
                    <div className={`p-2 rounded-lg ${getIconBgColor(project.color)} group-hover:scale-110 transition-transform duration-200`}>
                      <Icon size={20} />
                    </div>
                    <h3 className="text-lg font-bold text-slate-900">{project.title}</h3>
                  </div>

                  <p className="text-slate-700 mb-3 leading-relaxed text-sm">{project.description}</p>
